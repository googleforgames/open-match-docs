...
| ----- | ---- | ----- | ----------- |
| ticket_ids | [string](#string) | repeated | A list of ticket IDs generated by Open Match that will be assigned to a connection. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| config | [FunctionConfig](#openmatch.FunctionConfig) |  | A config that specifies where the MatchFunction server is for this FetchMatches call. |
| profile | [MatchProfile](#openmatch.MatchProfile) |  | A MatchProfile that describe what the returned Matches looks like for this FetchMatches call. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| match | [Match](#openmatch.Match) |  | A Match generated by the user-defined MMF with the specified MatchProfiles. |
...
| ----------- | ------------ | ------------- | ------------|
| FetchMatches | [FetchMatchesRequest](#openmatch.FetchMatchesRequest) | [FetchMatchesResponse](#openmatch.FetchMatchesResponse) stream | FetchMatches triggers a MatchFunction with the specified MatchProfile and returns a set of match proposals that match the description of that MatchProfile. FetchMatches method streams the results back to the caller and returns an error if it encounters any execution failures. - If the synchronizer is enabled, FetchMatch will then call the synchronizer to deduplicate proposals with overlapped tickets within configured registration window. |
...
| Method Name | Request Type | Response Type | Description |
| ----------- | ------------ | ------------- | ------------|
| Evaluate | [EvaluateRequest](#openmatch.EvaluateRequest) stream | [EvaluateResponse](#openmatch.EvaluateResponse) stream | Evaluate evaluates a list of proposed matches based on quality, collision status, and etc, then shortlist the matches and returns the match IDs of the final results. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| tickets | [Ticket](#openmatch.Ticket) | repeated | Tickets is a list of Ticket that meets all Filter criterias. |
